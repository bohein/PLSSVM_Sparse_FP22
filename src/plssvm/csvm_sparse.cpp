...
#include "plssvm/csvm_sparse.hpp"
...
template <typename T>
csvm_sparse<T>::csvm_sparse(const parameter_sparse<T> &params) :
    ...
    } else if (data_ptr_-> get_height() == 0) {
        throw exception{ "Data set is empty!" };
    }  else if (data_ptr_-> get_width() == 0) {
        throw exception{ "No features provided for the data points!" };
    } else if (alpha_ptr_ != nullptr && alpha_ptr_->size() != data_ptr_-> get_height()) {
        throw exception{ fmt::format("Number of weights ({}) must match the number of data points ({})!", alpha_ptr_->size(), data_ptr_->get_height()) };
    }

    num_data_points_ = data_ptr_->get_height();
    num_features_ = data_ptr_->get_width();
}

template <typename T>
void csvm_sparse<T>::write_model(const std::string &model_name) {
    ...
    } else if (data_ptr_->get_height() != value_ptr_->size()) {
        throw exception{ fmt::format("Number of labels ({}) must match the number of data points ({})!", value_ptr_->size(), data_ptr_->get_height()) };
    }
    PLSSVM_ASSERT(data_ptr_->get_height() == alpha_ptr_->size(), "Sizes mismatch!: {} != {}", data_ptr_->get_height(), alpha_ptr_->size());  // exception in constructor
    PLSSVM_ASSERT(data_ptr_->get_height() != 0, "Data set is empty!");                                                                    // exception in constructor
    PLSSVM_ASSERT(data_ptr_->get_width() != 0, "No features provided for the data points!");  // exception in constructor
...
template <typename T>
auto csvm_sparse<T>::accuracy() -> real_type {
    if (value_ptr_ == nullptr) {
        throw exception{ "No labels given! Maybe the data is only usable for prediction?" };
    }
    PLSSVM_ASSERT(data_ptr_ != nullptr, "No data is provided!");  // exception in constructor
...
template <typename T>
auto csvm_sparse<T>::accuracy(const std::vector<real_type> &point, const real_type correct_label) -> real_type {
    PLSSVM_ASSERT(data_ptr_ != nullptr, "No data is provided!");  // exception in constructor
    PLSSVM_ASSERT(data_ptr_->get_height() != 0, "Data set is empty!");     // exception in constructor
    if (point.size() != data_ptr_->get_width()) {
        throw exception{ fmt::format("Number of features per data point ({}) must match the number of features of the predict point ({})!", data_ptr_->get_width(), point.size()) };
    }
...
template <typename T>
auto csvm_sparse<T>::accuracy(const std::vector<std::vector<real_type>> &points, const std::vector<real_type> &correct_labels) -> real_type {
    ...
    } else if (points.front().size() != data_ptr_->get_width()) {
...
template <typename T>
auto csvm_sparse<T>::predict(const std::vector<real_type> &point) -> real_type {
    PLSSVM_ASSERT(data_ptr_ != nullptr, "No data is provided!");  // exception in constructor
    PLSSVM_ASSERT(data_ptr_->get_heigth() != 0, "Data set is empty!");     // exception in constructor
    if (point.size() != data_ptr_->get_width()) {
        throw exception{ fmt::format("Number of features per data point ({}) must match the number of features of the predict point ({})!", data_ptr_->get_width(), point.size()) };
    }
...
template <typename T>
auto csvm_sparse<T>::predict_label(const std::vector<real_type> &point) -> real_type {
    PLSSVM_ASSERT(data_ptr_ != nullptr, "No data is provided!");  // exception in constructor
    PLSSVM_ASSERT(data_ptr_->get_heigth() != 0, "Data set is empty!");     // exception in constructor
    if (point.size() != data_ptr_->get_width()) {
        throw exception{ fmt::format("Number of features per data point ({}) must match the number of features of the predict point ({})!", data_ptr_->get_width(), point.size()) };
...
template <typename T>
auto csvm_sparse<T>::predict_label(const std::vector<std::vector<real_type>> &points) -> std::vector<real_type> {
    PLSSVM_ASSERT(data_ptr_ != nullptr, "No data is provided!");  // exception in constructor
    PLSSVM_ASSERT(data_ptr_->get_heigth() != 0, "Data set is empty!");     // exception in constructor
    ...
    if (!std::all_of(points.begin(), points.end(), [&](const std::vector<real_type> &point) { return point.size() == points.front().size(); })) {
        ...
        throw exception{ fmt::format("Number of features per data point ({}) must match the number of features per predict point ({})!", data_ptr_->get_width(), points.front().size()) };
    }
...
template <typename T>
auto csvm_sparse<T>::kernel_function(const std::vector<real_type> &xi, const std::vector<real_type> &xj) -> real_type {
...
template class csvm_sparse<float>;
template class csvm_sparse<double>;
