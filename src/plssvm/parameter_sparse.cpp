...
#include "plssvm/parameter_sparse.hpp"
...
// read and parse file
template <typename T>
void parameter_sparse<T>::parse_file(const std::string &filename, std::shared_ptr<const plssvm::openmp::csr<real_type>> &data_ptr_ref) {
    if (detail::ends_with(filename, ".arff")) {
        throw std::invalid_argument(".arff is unsupported!");
    } else {
...
// read and parse file
template <typename T>
void parameter_sparse<T>::parse_file(const std::string &filename, std::shared_ptr<const plssvm::openmp::coo<real_type>> &data_ptr_ref) {
    if (detail::ends_with(filename, ".arff")) {
        throw std::invalid_argument(".arff is unsupported!");
    } else {
        parse_libsvm_file_sparse(filename, data_ptr_ref);
...
template <typename T>
void parameter_sparse<T>::parse_libsvm_file_sparse(const std::string &filename, std::shared_ptr<const plssvm::openmp::coo<real_type>> &data_ptr_ref) {
...
template <typename T>
void parameter_sparse<T>::parse_libsvm_file_sparse(const std::string &filename, std::shared_ptr<const plssvm::openmp::csr<real_type>> &data_ptr_ref) {
    auto start_time = std::chrono::steady_clock::now();
...
template <typename T>
void parameter_sparse<T>::parse_model_file(const std::string &filename) {
    auto start_time = std::chrono::steady_clock::now();
    ...
    plssvm::openmp::csr<real_type> data(num_sv);
    std::vector<real_type> alphas(num_sv);

    // parse support vectors
    detail::parse_libsvm_content_sparse(f, header + 1, data, alphas);

    // update shared pointer
    data_ptr = std::make_shared<const plssvm::openmp::csr<real_type>>(std::move(data));
...
template <typename T>
void parameter_sparse<T>::parse_train_file(const std::string &filename) {
...
template <typename T>
void parameter_sparse<T>::parse_test_file(const std::string &filename) {
    parse_file(filename, test_data_ptr);
}

template <typename T>
std::ostream &operator<<(std::ostream &out, const parameter_sparse<T> &params) {
...
template std::ostream &operator<<(std::ostream &, const parameter_sparse<float> &);
template std::ostream &operator<<(std::ostream &, const parameter_sparse<double> &);

template <typename T>
void parameter_sparse<T>::wrapper_for_parse_libsvm_content_sparse(const detail::file_reader &f, const std::size_t start, plssvm::openmp::coo<real_type> &data, std::vector<real_type> &values) {
    detail::parse_libsvm_content_sparse(f, start, data, values);
}

template <typename T>
void parameter_sparse<T>::wrapper_for_parse_libsvm_content_sparse(const detail::file_reader &f, const std::size_t start, plssvm::openmp::csr<real_type> &data, std::vector<real_type> &values) {
    detail::parse_libsvm_content_sparse(f, start, data, values);
}

template <typename T>
[[nodiscard]] std::string parameter_sparse<T>::model_name_from_input() {
    std::string::size_type pos = input_filename.find_last_of("/\\");
    return input_filename.substr(pos + 1) + ".model";
}

template <typename T>
[[nodiscard]] std::string parameter_sparse<T>::predict_name_from_input() {
    std::string::size_type pos = input_filename.find_last_of("/\\");
    return input_filename.substr(pos + 1) + ".predict";
}

// explicitly instantiate template class
template class parameter_sparse<float>;
template class parameter_sparse<double>;
