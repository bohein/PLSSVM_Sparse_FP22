#include "plssvm/backends/gpu_csvm_sparse.hpp"
...
gpu_csvm_sparse<T, device_ptr_t, queue_t>::gpu_csvm_sparse(const parameter_sparse<T> &params) :
    base_type{ params } {}

template <typename T, typename device_ptr_t, typename queue_t>
auto gpu_csvm_sparse<T, device_ptr_t, queue_t>::predict(const std::vector<std::vector<real_type>> &points) -> std::vector<real_type> {
