...
__global__ void device_kernel_linear(const real_type *q, real_type *ret, const real_type *d, const size_t *col_ids, const size_t *row_ids, const real_type *values, const real_type QA_cost, const real_type cost, const kernel_index_type nnz, const kernel_index_type width, const kernel_index_type height, const real_type add) {
    ...
    // padding before "values" = [ 0, ... , 0 ]
    // padding after "values" = [ nnz, ... , nnz ]
    ...
    // fill search indices
    kernel_index_type approx_pos = (nnz / height) * ij;
    if (row_ids[approx_pos] < ij) {
        approx_pos += threadIdx.x;
        while (row_ids[approx_pos] < ij) {
    ...
    approx_pos = (nnz / height) * j;
    if (row_ids[approx_pos] < j) {
        approx_pos += threadIdx.x;
        while (row_ids[approx_pos] < j) {
            approx_pos += THREAD_BLOCK_SIZE;
        }
    } else {
        approx_pos += threadIdx.x;
    ...
    if (threadIdx.y == 0) {
        ...
        approx_pos = (nnz / height) * last_row_id;
        if (row_ids[approx_pos] < last_row_id) {
            approx_pos += threadIdx.x;
            while (row_ids[approx_pos] < last_row_id) {
                approx_pos += THREAD_BLOCK_SIZE;
            }
        } else {
            approx_pos += threadIdx.x;
    ...
    while (global_entries_left) {
        ...
        for (kernel_index_type internal_index = 0; internal_index < INTERNAL_BLOCK_SIZE; ++internal_index) {
            ...
            if (match_1) {
                if (within_bounds_1) {
                    bc_col_ids_1[threadIdx.y][internal_index] = col_ids[row_beginnings_1[threadIdx.x] + row_offset_1 + internal_index];
                    bc_values_1[threadIdx.y][internal_index] = values[row_beginnings_1[threadIdx.x] + row_offset_1 + internal_index];
                } else {
                    bc_col_ids_1[threadIdx.y][internal_index] = 0;
                    bc_values_1[threadIdx.y][internal_index] = 0;
                }
            }
            ...
            if (match_2) {
                if (within_bounds_2) {
                   bc_col_ids_2[threadIdx.y][internal_index] = col_ids[row_beginnings_2[threadIdx.x] + row_offset_2 + internal_index];
                   bc_values_2[threadIdx.y][internal_index] = values[row_beginnings_2[threadIdx.x] + row_offset_2 + internal_index];
                } else {
                   bc_col_ids_2[threadIdx.y][internal_index] = 0;
                   bc_values_2[threadIdx.y][internal_index] = 0;
                }
            }
        }
