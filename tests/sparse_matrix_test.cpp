...
TYPED_TEST(SparseMatrix, coo_get_row) {
    using real_type = TypeParam;
    ...
    plssvm::openmp::coo<real_type> row0 = matrix.get_row(0);
    EXPECT_EQ(row0.get_nnz(), 3);
...
TYPED_TEST(SparseMatrix, csr_get_last_row_begin) {
    using real_type = TypeParam;
