...
#include "plssvm/parameter_sparse.hpp"  // plssvm::parameter
 ...
 */
template <typename T>
class parameter_train_sparse : public parameter_sparse<T> {
  public:
    /// The template base type of the parameter_train class.
    using base_type = parameter_sparse<T>;
     ...
     */
    parameter_train_sparse() = default;
     ...
     */
    explicit parameter_train_sparse(std::string input_filename);
     ...
     */
    parameter_train_sparse(int argc, char **argv);
};

extern template class parameter_train_sparse<float>;
extern template class parameter_train_sparse<double>;
