...
#include "plssvm/csvm_sparse.hpp"  // plssvm::csvm
...
class gpu_csvm_sparse : public csvm_sparse<T> {
  protected:
    /// The template base type of the C-SVM class.
    using base_type = ::plssvm::csvm_sparse<T>;
  ...
  public:
     ...
     */
    explicit gpu_csvm_sparse(const parameter_sparse<T> &params);

    /**
     * @brief Virtual destructor to enable safe inheritance.
     */
    virtual ~gpu_csvm_sparse() = default;
